...
def plot_epipolar_lines(image1, image2, uncalibrated_1, uncalibrated_2, E, K, plot=True):
    ...
    F = K_inv.T @ E @ K_inv
    epipolar_lines_in_1 = np.zeros((3, uncalibrated_1.shape[1]))
    for i in range(uncalibrated_1.shape[1]):
        p = uncalibrated_2[:, i]
        epipolar_lines_in_1[:, i] = p.T @ F

    F2 = K_inv.T @ E.T @ K_inv
    epipolar_lines_in_2 = np.zeros((3, uncalibrated_2.shape[1]))
    for i in range(uncalibrated_2.shape[1]):
        p2 = uncalibrated_1[:, i]
        epipolar_lines_in_2[:, i] = p2.T @ F2
